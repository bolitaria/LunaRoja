...
export default function CampanaDetalle() {
  ...
  const tileClassName = ({ date, view }) => {
    ...
    if (selectedDate && date.toDateString() === selectedDate.toDateString()) {
      return 'bg-green-600 text-white rounded-2xl';
    }
    return null;
  };
  ...
  return (
    <Layout title={`${campaign.name} - Voces Palestinas por la Justicia`}>
      {/* Contenedor sin margen inferior extra para que el espacio sea fino y simétrico */}
      <div className="container mx-auto px-4 lg:px-8 py-8 max-w-7xl bg-white rounded-xl shadow-sm border border-gray-200">
        {/* Encabezado con badge de color al lado del título */}
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-8 pb-4 border-b border-gray-200">
          <div className="flex items-center gap-2">
            ...
            {/* Label de color más larga (w-16) */}
            <span
              className="inline-block w-16 h-8 border border-gray-300 shadow-sm flex-shrink-0 rounded"
          ...
          <div className="flex flex-wrap items-center gap-2">
            {campaign.active ? (
              <span className="inline-block px-3 py-1 bg-green-100 text-green-800 rounded-lg text-sm font-medium">Activa</span>
            ) : (
              <span className="inline-block px-3 py-1 bg-gray-100 text-gray-600 rounded-lg text-sm font-medium">Inactiva</span>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="space-y-4">
            {campaign.imageUrl ? (
              ...
              <div className="w-full aspect-video bg-gray-100 rounded-xl flex items-center justify-center text-gray-400 border-2 border-dashed border-gray-300">
                <svg className="w-16 h-16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <span className="ml-2 text-sm font-medium">Sin imagen</span>
              </div>
            )}
        ...
        {/* Sección de acciones y calendario con degradado amarillo como en la página de peticiones */}
        <div className="mt-8 bg-gradient-to-b from-yellow-100 via-amber-50 to-white rounded-xl shadow-sm border border-gray-200 px-4 lg:px-8 py-4">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg border font-medium text-xs transition ${
                  timeFilter === 'todas'
                    ? 'bg-white text-fuchsia-600 border-2 border-fuchsia-600 shadow-sm'
              ...
              {selectedDate && (
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg border text-xs bg-white text-gray-700 border-gray-300 hover:bg-gray-50 transition"
                >
                  ✕ Limpiar fecha
                </button>
              )}
                  ...
                  {filteredActions.map(action => {
                    ...
                    return (
                      <Link key={action.id} href={`/acciones/${action.id}`} className="block">
                        <div className={`border border-gray-200 rounded-lg p-3 hover:shadow-md transition ${isPast ? 'bg-gray-50' : 'bg-white'}`}>
                          <div className="flex justify-between items-start gap-2">
                            <h3 className="font-semibold text-gray-700 text-sm">{action.title}</h3>
                            <span className="text-xs px-2 py-0.5 rounded-full border bg-purple-100 text-purple-800 border-purple-200 whitespace-nowrap">
                              {catLabel}
                            </span>
                          </div>
                          <div className="flex flex-wrap items-center gap-2 mt-1 text-xs text-gray-500">
                            ...
                            <span className={`ml-auto inline-flex items-center px-2 py-0.5 rounded-full text-xs ${isOnline ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
      ...
      {/* Estilos globales para ocultar puntos y otros elementos no deseados en el calendario */}
      <style jsx global>{`
        .react-calendar__tile .react-calendar__tileContent {
          display: none !important;
        }
        .react-calendar__tile div:not(:first-child) {
          display: none !important;
        }
