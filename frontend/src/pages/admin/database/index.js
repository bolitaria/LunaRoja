...
import { FaDownload, FaSync, FaSearch, FaEye } from 'react-icons/fa';
    ...
    <AdminLayout title="Base de Datos">
      ...
      <div className="flex justify-between items-center mb-6 border-b border-gray-200 pb-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleBackup} className="inline-flex items-center gap-1.5 text-sm border border-green-300 text-green-700 bg-white px-3 py-1.5 rounded-lg hover:bg-green-50 transition-colors">
            <FaDownload className="w-3.5 h-3.5" /> Backup
          </button>
          <button onClick={handleRunMigrations} disabled={executingMigrations} className="inline-flex items-center gap-1.5 text-sm border border-amber-300 text-amber-700 bg-white px-3 py-1.5 rounded-lg hover:bg-amber-50 transition-colors disabled:opacity-50">
              ...
              {paginated.map(table => (
                <tr key={table.tablename} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <button onClick={() => fetchTableIndexes(table.tablename)} className="text-gray-400 hover:text-fuchsia-600 transition-colors" title="Ver índices">
