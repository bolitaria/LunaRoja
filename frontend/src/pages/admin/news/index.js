...
function AdminNews() {
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!confirm(`¿Eliminar ${selected.length} noticias?`)) return;
    try {
      await Promise.all(selected.map(id => api.delete(`/news/${id}`)));
      toast.success(`${selected.length} noticias eliminadas`);
      setSelected([]);
      fetchNews();
    } catch (err) {
      toast.error('Error al eliminar');
  ...
  const toggleSelectAll = (e) => {
    if (e.target.checked) setSelected(paginated.map(n => n.id));
    else setSelected([]);
  };
            ...
            <tbody className="divide-y divide-purple-100">
              {paginated.map(n => (
                <tr key={n.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4"><input type="checkbox" checked={selected.includes(n.id)} onChange={() => toggleOne(n.id)} /></td>
                  <td className="px-6 py-4 font-medium text-gray-900">{n.title}</td>
                  <td className="px-6 py-4 hidden sm:table-cell text-gray-500">
                    {n.campaign?.name || n.action?.title || '-'}
                  </td>
                  <td className="px-6 py-4 hidden md:table-cell text-gray-500">{new Date(n.publishedAt).toLocaleString()}</td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-1">
                      <Link href={`/admin/news/${n.id}/edit`} className="p-1.5 text-gray-400 hover:text-fuchsia-600"><FaEdit /></Link>
                      <button onClick={() => handleDelete(n.id)} className="p-1.5 text-gray-400 hover:text-red-600"><FaTrash /></button>
                    </div>
                  </td>
                </tr>
              ))}
