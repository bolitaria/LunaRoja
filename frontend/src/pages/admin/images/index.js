...
function AdminImages() {
  ...
  const fetchImages = async () => {
    try { const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/images`, { headers: { Authorization: `Bearer ${token}` } }); setImages(res.data); }
  ...
  const executeDelete = async () => {
    ...
    try {
      await Promise.all(ids.map(id => axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/images/${id}`, { headers: { Authorization: `Bearer ${token}` } })));
    ...
    <AdminLayout title="Imágenes">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: 'Acciones', value: actionImages, color: 'bg-blue-100 text-blue-800' },
          { label: 'Reportes', value: reportImages, color: 'bg-orange-100 text-orange-800' },
        ].map((m, i) => (
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <select value={filterType} onChange={(e) => { setFilterType(e.target.value); setCurrentPage(1); }} className="border border-gray-200 rounded-lg px-3 py-1.5 text-xs bg-gray-50 text-gray-600">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {paginated.map(img => (
              <div key={img.id} className="group relative bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="p-2">
                  <p className="text-xs text-gray-500 truncate">{img.relatedTitle || `ID: ${img.relatedId}`}</p>
                  {img.campaign && (
                    <div className="flex items-center gap-1 mt-1">
                      <span className="w-2 h-2 rounded-full" style={{ backgroundColor: img.campaign.color }} />
                      <span className="text-xs text-gray-400 truncate">{img.campaign.name}</span>
                    </div>
                  )}
                </div>
          ...
          <table className="min-w-full divide-y divide-gray-100 text-sm">
            <thead className="bg-gray-50 text-gray-500 uppercase tracking-wider">
              <tr>
                ...
                <th className="px-6 py-3 text-left hidden md:table-cell">Relacionado</th>
