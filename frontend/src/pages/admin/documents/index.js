...
function AdminDocuments() {
  ...
  const fetchDocuments = async () => {
    try { const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/documents`, { headers: { Authorization: `Bearer ${token}` } }); setDocuments(res.data); }
    catch (error) { toast.error('Error al cargar documentos'); } finally { setLoading(false); }
  };
  ...
  useEffect(() => { Promise.all([fetchDocuments(), fetchCampaigns(), fetchActions()]).then(() => setLoading(false)); }, []);
  ...
  const executeDelete = async () => {
    ...
    try {
      await Promise.all(ids.map(id => axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/documents/${id}`, { headers: { Authorization: `Bearer ${token}` } })));
      toast.success(`${ids.length} documento(s) eliminado(s)`); setSelected([]); fetchDocuments();
  ...
  const exportCSV = () => {
    ...
    const data = filtered.map(doc => ({
      title: doc.title, visibility: doc.type === 'public' ? 'Público' : 'Privado',
  ...
  const toggleSelectAll = (e) => { if (e.target.checked) setSelected(paginated.map(d => d.id)); else setSelected([]); };
  const toggleOne = (id) => setSelected(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);

  return (
    <AdminLayout title="Documentos">
      <ToastContainer />
      <ConfirmModal isOpen={showDeleteModal} title="Eliminar documento" message={deleteTarget && (Array.isArray(deleteTarget) ? `¿Eliminar ${deleteTarget.length} documentos seleccionados?` : '¿Eliminar este documento?')} onConfirm={executeDelete} onCancel={() => { setShowDeleteModal(false); setDeleteTarget(null); }} />

      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Total', value: total, color: 'bg-cyan-100 text-cyan-800' },
          { label: 'Públicos', value: publicCount, color: 'bg-green-100 text-green-800' },
          { label: 'Privados', value: privateCount, color: 'bg-yellow-100 text-yellow-800' },
        ].map((m, i) => (
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <select value={filterActionId} onChange={(e) => { setFilterActionId(e.target.value); setCurrentPage(1); }} className="border border-gray-200 rounded-lg px-3 py-1.5 text-xs bg-gray-50 text-gray-600">
          <option value="">Acción</option>
          {actions.map(a => <option key={a.id} value={a.id}>{a.title}</option>)}
        </select>
        <select value={filterVisibility} onChange={(e) => { setFilterVisibility(e.target.value); setCurrentPage(1); }} className="border border-gray-200 rounded-lg px-3 py-1.5 text-xs bg-gray-50 text-gray-600">
          <option value="">Visibilidad</option>
          <option value="public">Público</option>
          <option value="organizer">Privado</option>
        </select>
      </div>

      {loading ? <p className="text-gray-500 text-sm">Cargando...</p> : filtered.length === 0 ? <p className="text-gray-500 text-sm">No se encontraron documentos.</p> : (
              ...
              {paginated.map(doc => {
                ...
                return (
                  <tr key={doc.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 hidden md:table-cell text-gray-500">{action ? action.title : '-'}</td>
                    <td className="px-6 py-4 text-gray-500">{new Date(doc.createdAt).toLocaleDateString()}</td>
                    <td className="px-6 py-4"><div className="flex items-center gap-2"><a href={`${process.env.NEXT_PUBLIC_BASE_URL}${doc.fileUrl}`} target="_blank" rel="noopener" className="text-gray-400 hover:text-blue-600"><FaEye /></a><button onClick={() => handleDelete(doc.id)} className="text-gray-400 hover:text-red-600"><FaTrash /></button></div></td>
                  </tr>
                );
              })}
            </tbody>
...
export default withAuth(AdminDocuments);