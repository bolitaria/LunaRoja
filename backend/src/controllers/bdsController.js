...
const { sendCampaignNotification } = require('../services/emailService'); // o uno propio
...
exports.createBDS = async (req, res) => {
  try {
    ...
    // Notificar a suscriptores (opcional)
    try {
      const subscribers = await Subscriber.findAll({ where: { status: 'active' } });
      for (const sub of subscribers) {
        await sendCampaignNotification(sub.email, bds).catch(err => console.error(`Error email a ${sub.email}:`, err));
      }
