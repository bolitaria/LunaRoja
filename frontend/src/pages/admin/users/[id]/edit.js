...
export default function EditUser() {
  ...
  if (!canEdit) return <AdminLayout title="Editar Usuario"><div className="text-center py-12 text-red-600"><p className="text-lg">No tienes permisos para editar este usuario.</p></div></AdminLayout>;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select
              ...
              className="mt-1 block w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-fuchsia-500 focus:border-transparent disabled:bg-gray-100"
            >
              {['superadmin', 'campaign_admin', 'action_admin', 'bds_admin'].map(role => (
          ...
          {(currentUser?.role === 'superadmin' || currentUser?.role === 'campaign_admin') && form.role === 'campaign_admin' && (
            <div>
