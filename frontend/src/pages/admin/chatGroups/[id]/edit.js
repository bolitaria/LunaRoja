...
export default function EditChatGroup() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        let associationType = '';
        let campaignId = g.campaignId || '';
        let bdsId = g.bdsId || '';
        let actionId = g.actionId || '';
        if (g.campaignId) associationType = 'campaign';
        else if (g.bdsId) associationType = 'bds';
        else if (g.actionId) associationType = 'action';
  ...
  if (initialLoading) {
    return (
      <AdminLayout title="Editar Grupo de Chat">
    ...
    <AdminLayout title="Editar Grupo de Chat">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border-2 border-gray-300 p-6 space-y-6">
        <h2 className="text-xl font-semibold text-gray-800 flex items-center gap-2">
          <FaSave className="text-fuchsia-600" />
