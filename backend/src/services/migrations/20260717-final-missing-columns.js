module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    // Petitions (todas las columnas que faltan)
    ...
    // Email Quota
    await query(`CREATE TABLE IF NOT EXISTS "email_quota" ("date" DATE PRIMARY KEY, "sent_count" INTEGER DEFAULT 0)`);
  ...
  async down(queryInterface) {
    const { sequelize } = queryInterface;
    const query = sequelize.query.bind(sequelize);
    // Eliminar columnas y tablas en orden inverso (opcional, solo si se necesita revertir)
  },
};
