...
function EditAction() {
  ...
  const loadLeaflet = () => {
    ...
    if (window.L) { setMapApiLoaded(true); return; }
    ...
    script.async = true;
    script.onload = () => { setMapApiLoaded(true); scriptLoadingRef.current = false; };
    script.onerror = () => { toast.error('Error al cargar el mapa'); scriptLoadingRef.current = false; };
    ...
    const fetchData = async () => {
      try {
        ...
        setForm({
          ...
          bdsId: action.bdsId || '',
          privateLink: action.privateLink || '',
        });

        if (action.campaignId) setLinkType('campaign');
        else if (action.bdsId) setLinkType('bds');
        ...
        else toast.error(error.response?.data?.message || 'Error al cargar la acción');
      }
    };
    fetchData();
    loadLeaflet();
  ...
  const searchAddress = async () => {
    if (!form.address.trim()) { toast.warning('Escribe una dirección para buscar.'); return; }
    try {
      ...
      if (data.length > 0) {
        ...
        setForm(prev => ({ ...prev, latitude: lat, longitude: lon, address: display_name, placeName: !prev.placeName ? display_name.split(',')[0] : prev.placeName }));
        toast.success('Dirección encontrada.');
      } else toast.error('No se encontró la dirección.');
    } catch (error) { toast.error('Error al buscar la dirección.'); }
  };
  const handleOpenMapModal = () => { if (!mapApiLoaded) loadLeaflet(); setShowMapModal(true); };
  ...
  useEffect(() => {
    ...
    if (form.latitude && form.longitude) markerRef.current = L.marker([parseFloat(form.latitude), parseFloat(form.longitude)]).addTo(map);
    map.on('click', async (e) => {
      ...
      try {
        ...
        let placeName = '';
        if (data.address) placeName = data.address.amenity || data.address.road || data.address.suburb || '';
        setForm(prev => ({ ...prev, address, latitude: lat, longitude: lng, placeName: placeName || prev.placeName }));
        toast.success('Ubicación seleccionada correctamente.');
        setShowMapModal(false);
      } catch (error) { toast.error('Error al obtener la dirección.'); }
    });
  ...
  useEffect(() => {
    ...
    if (previewMapRef.current._leaflet_id) { const oldMap = previewMapRef.current._leaflet_map; if (oldMap) oldMap.remove(); }
    const map = L.map(previewMapRef.current, { center: [parseFloat(form.latitude), parseFloat(form.longitude)], zoom: 15, zoomControl: false, dragging: false, scrollWheelZoom: false, doubleClickZoom: false, touchZoom: false, keyboard: false });
  ...
  const handleFeaturedImageChange = (e) => {
    ...
    if (file) { setFeaturedImageFile(file); const reader = new FileReader(); reader.onloadend = () => setFeaturedImagePreview(reader.result); reader.readAsDataURL(file); }
  };
  const handleNewImages = (e) => {
    const files = Array.from(e.target.files);
    if (newImageFiles.length + files.length > 20) { toast.warning(`Máximo 20 imágenes. Ya tienes ${newImageFiles.length}.`); return; }
  ...
  const handleDeleteImage = async (imageId) => {
    ...
    try { await api.delete(`/actions/images/${imageId}`); toast.success('Imagen eliminada'); setExistingImages(prev => prev.filter(img => img.id !== imageId)); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      else { formData.append('campaignId', ''); formData.append('bdsId', ''); }
  ...
  const getDirectionsUrl = () => {
    if (form.latitude && form.longitude) return `https://www.openstreetmap.org/directions?from=&to=${form.latitude},${form.longitude}`;
    ...
    <AdminLayout title="Editar Acción">
      <div className="flex flex-col lg:flex-row gap-8">
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-sm lg:w-2/3 space-y-6">
                    ...
                    <>
                      ...
                      <div className="md:col-span-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          <input type="text" placeholder="Buscar dirección..." className={`flex-1 ${inputClass}`} value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} />
                          <button type="button" onClick={searchAddress} className="px-3 py-2 border border-gray-300 rounded-lg text-base text-gray-600 hover:bg-gray-50">Buscar</button>
                          <button type="button" onClick={handleOpenMapModal} className="px-3 py-2 border border-gray-300 rounded-lg text-base text-gray-600 hover:bg-gray-50 hover:border-gray-400 transition-colors flex items-center gap-1">
                      ...
                      {form.latitude && form.longitude && (
                        <div className="md:col-span-2 mt-2">
                          ...
                          <div className="flex gap-2 mt-2">
                            <a href={getDirectionsUrl()} target="_blank" rel="noopener noreferrer" className="text-sm px-3 py-1 rounded bg-green-100 text-green-800 hover:bg-green-200 transition-colors flex items-center gap-1">
                              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
                              Cómo llegar
                            </a>
                            <button type="button" onClick={handleOpenMapModal} className="text-sm px-3 py-1 rounded bg-fuchsia-100 text-fuchsia-800 hover:bg-fuchsia-200 transition-colors flex items-center gap-1">
          ...
          <div className="border-l-2 border-green-500 pl-4 mt-4 relative">
            <span className="absolute -left-[5px] top-2 w-2.5 h-2.5 rounded-full bg-green-500"></span>
            <h3 className="text-lg font-semibold text-gray-700 flex items-center gap-2"><span>📸 </span> Galería de imágenes</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-base font-medium text-gray-700 mb-1">Galería de imágenes</label>
                ...
                <div className="grid grid-cols-4 gap-4">
                  {newImagePreviews.map((src, idx) => (
                    <div key={idx} className="relative group">
                      ...
                      <button type="button" onClick={() => removeNewImage(idx)} className="absolute -top-1 -right-1 bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-700 transition-colors">✕</button>
                    </div>
                  ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white rounded-xl max-w-4xl w-full p-6 shadow-2xl">
            ...
            <div className="flex gap-2 mb-4">
              <input type="text" id="modal-search-input" placeholder="Buscar dirección..." className={`flex-1 ${inputClass}`}
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
              ...
              }} className="px-4 py-2 bg-fuchsia-600 text-white rounded-lg hover:bg-fuchsia-700 transition-colors text-base">Buscar</button>
            </div>
            {!mapApiLoaded ? (
              <div className="flex items-center justify-center h-64 bg-gray-100 rounded-lg"><p className="text-gray-500">Cargando mapa...</p></div>
            ) : (
